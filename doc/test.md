...
| 2   | malloc         | size-0-behaviour             | `p = malloc(0)`                                        | `(p==NULL)` v `(unique pointer ∧ free(p) safe)`             |
...
| 7   | malloc         | huge-but-legal               | `malloc(RLIMIT_AS/4)`                                  | success v `(NULL ∧ errno==ENOMEM)`                          |
...
| 17  | zones          | at-least-100-allocs-per-zone | loop `malloc(n-1)` 100x                                | ≤ 1 `mmap` call (via strace)                                |
